/*
 * @file            ReceiveMeasurement.cpp
 * @brief           Demonstrates receiving tool measurement data via GoPxL Data Protocol (GDP)
 *
 * @details         This sample shows how to receive measurement data from measurement tools.
 *                  It demonstrates how to:
 *                  - Enable GoPxL Data Protocol (GDP)
 *                  - Connect to GDP server
 *                  - Receive measurement data synchronously via GDP
 *                  - Process measurement values and pass/fail decisions
 *                  - Handle NULL messages when measurements are invalid
 *
 *                  Prerequisites: Tool measurement outputs must be enabled and added to GDP
...
int ReceiveMeasurement()
{
    ...
    catch (const std::exception& e)
    {
        std::cerr << "Error: " << e.what()
            ...
            << "Ensure measurement data is available as output or try increasing timeout value."
            << std::endl;
        return ERROR_STATUS;
    }
    ...
    for (size_t msgIndex = 0; msgIndex < receivedDataSet.Count(); msgIndex++)
    {
        ...
        switch (msg.Type())
        {
            ...
            {
                const auto& measurementMsg = static_cast<const GoGdpMeasurement&>(msg);
                std::cout << "Message type: Measurement" << std::endl;
                std::cout << "GDP ID: " << measurementMsg.GdpId() << std::endl;
                std::cout << "Data source ID: " << measurementMsg.DataSourceId() << std::endl;

                if (measurementMsg.ArrayedCount() == 0) {
                    std::cout << "Not arrayed" << std::endl;
                }
                else {
                    std::cout << "Arrayed" << std::endl;
                    std::cout << "\tCount: " << measurementMsg.ArrayedCount() << std::endl;
                    std::cout << "\tIndex: " << measurementMsg.ArrayedIndex() << std::endl;
                }

                std::cout << "\tValue: " << measurementMsg.Value() << std::endl;
                std::cout << "\tDecision: " << static_cast<int>(measurementMsg.Decision()) << std::endl;
                break;
            }
            ...
            default:
                std::cout << "No measurement found in the message." << std::endl;
                break;
        }
    }
