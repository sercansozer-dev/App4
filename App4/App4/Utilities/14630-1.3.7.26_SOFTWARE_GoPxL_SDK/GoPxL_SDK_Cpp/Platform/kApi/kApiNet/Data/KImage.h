...
namespace Lmi3d
{
    ...
    {       
        namespace Data
        {
            ...
            ///   <item><description>Advanced formats are supported; set the pixel type to kVoid and format >= kPIXEL_FORMAT_VOID_TYPE_START.</description></item>
            /// </list>
            /// </para>
            ///
            ...
            public:
                ...
                /// <param name="allocator">Memory allocator.</param>
                /// <param name="alignment">Memory alignment for data.</param>
                KImage(KType^ pixelType, KPixelFormat format, k64s width, k64s height, KAlloc^ allocator, KMemoryAlignment alignment)
                    : KObject(DefaultRefStyle)
                {
                    kImage handle = kNULL;

                    KCheck(kImage_ConstructEx(&handle, KToHandle(pixelType), format, (kSize)width, (kSize)height, KToHandle(allocator), KToHandle(allocator), (kMemoryAlignment)alignment));
