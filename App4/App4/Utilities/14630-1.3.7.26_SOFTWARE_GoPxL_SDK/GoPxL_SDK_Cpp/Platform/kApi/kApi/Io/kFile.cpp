 ...
 *
 * @internal
 * Copyright (C) 2004-2025 by LMI Technologies Inc.
...
{
    kObj(kFile, file); 
    k32u chunkSize = xkFILE_MAX_IO_SIZE;
    k64u remainingReadCount = maxCount;
    k64u readOffset = 0;
    DWORD readCount = 0;
    DWORD actualReadCount = 0;

    kCheckArgs(!kIsNull(buffer));

    while (remainingReadCount > 0)
    {
        readCount = (DWORD)kMin_(remainingReadCount, chunkSize);

        kCheck(ReadFile(obj->handle, buffer + readOffset, readCount, &actualReadCount, kNULL));
        ...
        readOffset += actualReadCount;
        remainingReadCount -= actualReadCount;
    }

    kCheckTrue((readOffset >= minCount), kERROR_STREAM);

    if (!kIsNull(bytesRead))
    {
        *bytesRead = (kSize)readOffset;
    }
...
kFx(kStatus) xkFile_WriteImpl(kFile file, const kByte* buffer, kSize count)
{
    ...
    while (left > 0)
    {
        chunkSize = (DWORD)kMin_(xkFILE_MAX_IO_SIZE, left);

        kCheck(WriteFile(obj->handle, buffer + offset, chunkSize, &written, kNULL));

        offset += written;
        left -= written;
    }

    kCheck(offset == (k64u)count); 
...
{
    kObj(kFile, file); 
    kSize maxIoSize = xkFILE_MAX_IO_SIZE;
    k64u remainingReadCount = maxCount;
    k64u readOffset = 0;
    k32u readCount = 0;
    kSSize actualReadCount = 0;

    kCheckArgs(!kIsNull(buffer));

    while (remainingReadCount > 0)
    {
        readCount = (k32u)kMin_(remainingReadCount, maxIoSize);

        if ((actualReadCount = read(obj->handle, buffer + readOffset, readCount)) < 0)
        {
...
kFx(kStatus) xkFile_WriteImpl(kFile file, const kByte* buffer, kSize count)
{
    ...
    size_t writeSize = 0;
    ssize_t written = 0;
    k64u remainingCount = count;
    k64u offset = 0;

    while (remainingCount > 0)
    {
        writeSize = kMin_(xkFILE_MAX_IO_SIZE, remainingCount);

        if ((written = write(obj->handle, buffer + offset, writeSize)) < 0)
        {
            return kERROR_STREAM;
        }
