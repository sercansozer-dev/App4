...
kFx(const kChar*) kStatus_Name(kStatus status)
{
    ...
    case kERROR_OVERHEAT:           return "kERROR_OVERHEAT";
...
kBeginValueEx(k, kWindow32u)
    kAddPrivateVersionEx(kWindow32u, "kdat6", "10.3.0.0", "kWindow32u-0", Write, Read)

    kAddField(kWindow32u, k32u, left)
    kAddField(kWindow32u, k32u, top)
    kAddField(kWindow32u, k32u, width)
    kAddField(kWindow32u, k32u, height)
kEndValueEx()

kFx(kBool) xkWindow32u_VEquals(kType type, const void* value, const void* other)
{
    const kWindow32u* a = (const kWindow32u*)value;
    const kWindow32u* b = (const kWindow32u*)other;

    return (a->left == b->left) && (a->top == b->top) && (a->width == b->width) && (a->height == b->height);
}

kFx(kSize) xkWindow32u_VHashCode(kType type, const void* value)
{
    const kWindow32u* v = (const kWindow32u*)value;

    return (kSize)(v->left ^ v->top ^ v->width ^ v->height);
}

kFx(kStatus) xkWindow32u_Write(kType type, const void* values, kSize count, kSerializer serializer)
{
    return kSerializer_Write32uArray(serializer, (const k32u*)values, 4 * count);
}

kFx(kStatus) xkWindow32u_Read(kType type, void* values, kSize count, kSerializer serializer)
{
    return kSerializer_Read32uArray(serializer, (k32u*)values, 4 * count);
}
...
kBeginEnumEx(k, kPixelFormat)
    ...
    kAddEnumerator(kPixelFormat, kPIXEL_FORMAT_MONO8)
    kAddEnumerator(kPixelFormat, kPIXEL_FORMAT_MONO12_PACKED)
...
kFx(kSize) xkPixelFormat_PixelBits(kPixelFormat format)
{
    switch (format)
    {
    case kPIXEL_FORMAT_NULL:                return 0;
    case kPIXEL_FORMAT_8BPP_GREYSCALE:
    case kPIXEL_FORMAT_8BPP_CFA:            return 8;
    case kPIXEL_FORMAT_8BPC_BGRX:           return 32;
    case kPIXEL_FORMAT_1BPP_GREYSCALE:      return 1;
    case kPIXEL_FORMAT_16BPP_GREYSCALE:     return 16;
    case kPIXEL_FORMAT_8BPC_BGR:            return 24;
    default:                                return (format >> 16) & 0xFF;
    }
}

kFx(kSize) xkPixelFormat_Stride(kPixelFormat format, kSize width)
{
    return ((width * xkPixelFormat_PixelBits(format)) >> 3);
}

kFx(kSize) xkPixelFormat_Width(kPixelFormat format, kSize stride)
{
    return ((stride << 3) / xkPixelFormat_PixelBits(format));
}

kFx(kBool) xkPixelFormat_UseVoidPixel(kPixelFormat format)
{
    return (format >= kPIXEL_FORMAT_VOID_TYPE_START);
}
