...
kFx(kStatus) xkNetworkAdapter_Construct(kNetworkAdapter* adapter, kNetworkInfo info, const kChar* name, kSize id, kBool isEnabled, kBool isLinkRunning, kNetworkAdapterType adapterType, kMacAddress macAddress, kIpAssignment ipAssignment, kAlloc allocator)
{
    kAlloc alloc = kAlloc_Fallback(allocator);
    kStatus status; 
    ...
    if (!kSuccess(status = xkNetworkAdapter_Init(*adapter, kTypeOf(kNetworkAdapter), info, name, id, isEnabled, isLinkRunning, adapterType, macAddress, ipAssignment, alloc)))
    {
...
kFx(kStatus) xkNetworkAdapter_Init(kNetworkAdapter adapter, kType type, kNetworkInfo info, const kChar* name, kSize id, kBool isEnabled, kBool isLinkRunning, kNetworkAdapterType adapterType, kMacAddress macAddress, kIpAssignment ipAssignment, kAlloc alloc)
{
    ...
    obj->isLinkRunning = isLinkRunning;
...
kFx(kStatus) kNetworkInfo_ConstructEx(kNetworkInfo* info, kNetworkAdapterType typeFilter, kNetworkAdapterState stateFilter, kAlloc allocator)
{
    kAlloc alloc = kAlloc_Fallback(allocator);
    kStatus status; 
    ...
    if (!kSuccess(status = xkNetworkInfo_Init(*info, kTypeOf(kNetworkInfo), typeFilter, stateFilter, alloc)))
    {
...
kFx(kStatus) xkNetworkInfo_Init(kNetworkInfo info, kType type, kNetworkAdapterType typeFilter, kNetworkAdapterState stateFilter, kAlloc alloc)
{
    ...
    kTry
    {
        ...
        kTest(kNetworkInfo_RefreshEx(info, typeFilter, stateFilter));
    }
    kCatch(&status); 
    {
...
kFx(kStatus) kNetworkInfo_RefreshEx(kNetworkInfo info, kNetworkAdapterType typeFilter, kNetworkAdapterState stateFilter)
{
    kObj(kNetworkInfo, info); 
    ...
    //enumerate adapters
    kCheck(kApiLib_QueryNetInfoHandler()(info, typeFilter, stateFilter));
...
{
    return kNetworkInfo_RefreshEx(info, kNETWORK_ADAPTER_TYPE_FILTER_DEFAULT, kNETWORK_ADAPTER_STATE_FILTER_DEFAULT);
}

kFx(kStatus) xkNetworkInfo_AddAdapter(kNetworkInfo info, const kChar* name, kSize id, kBool isEnabled, kBool isLinkRunning, kNetworkAdapterType adapterType, kMacAddress macAddress, kIpAssignment ipAssignment, kNetworkAdapter* adapter)
{    
    kObj(kNetworkInfo, info); 
    ...
    kTry
    {
        kTest(xkNetworkAdapter_Construct(&newAdapter, adapter, name, id, isEnabled, isLinkRunning, adapterType, macAddress, ipAssignment, kObject_Alloc(info)));
...
static kNetworkAdapterType xkNetwork_AdapterTypeFromIfType(IFTYPE ifType)
{
    switch (ifType)
    {
    case IF_TYPE_SOFTWARE_LOOPBACK:     return kNETWORK_ADAPTER_LOOPBACK;
    default:                            return kNETWORK_ADAPTER_ETHERNET;
    }
}

kFx(kStatus) xkNetwork_DefaultQueryNetInfo(kNetworkInfo info, xkNetworkInfoFilter typeFilter, xkNetworkInfoFilter stateFilter)
{
    ...
    kTry
    {
        ...
        while (!kIsNull(it))
        {
            ...
            kNetworkAdapter adapter = kNULL;
            kText128 name;
            kBool isEnabled = kTRUE; //enumeration via GetIfTable2 would be needed to get disabled interfaces (MIB_IF_ROW2.AdminStatus).
            kBool isLinkRunning = (it->OperStatus == IfOperStatusUp);
            kNetworkAdapterType adapterType = xkNetwork_AdapterTypeFromIfType(it->IfType);
            ...
            if ((adapterType & typeFilter) != 0)
            {
                kTest(xkNetworkInfo_AddAdapter(info, name, it->IfIndex, isEnabled, isLinkRunning, adapterType, macAddress, ipAssignment, &adapter));
            }
...
#               endif
                ...
                if (!kIsNull(adapter) && kSuccess(xkIpAddress_FromSockAddr(addr, &address, kNULL)))
                {
...
static kNetworkAdapterType xkNetwork_AdapterTypeFromFamType(sa_family_t familyType)
{
    switch (familyType)
    {
    case ARPHRD_LOOPBACK:           return kNETWORK_ADAPTER_LOOPBACK;
    default:                        return kNETWORK_ADAPTER_ETHERNET;
    }
}
        ...
        while (!kIsNull(ifaListIt))
        {
            ...
            if ((ifaListIt->ifa_addr != kNULL))
            {
                if (!kSuccess(kNetworkInfo_FindAdapterByName(info, name, &adapter)))
                {
                    ...
                    kBool isEnabled = ((ifaListIt->ifa_flags & IFF_UP) != 0);
                    kBool isLinkRunning = ((ifaListIt->ifa_flags & IFF_RUNNING) != 0);
                    ...
                    {
                        kNetworkAdapterType adapterType = xkNetwork_AdapterTypeFromFamType(ifaceReq.ifr_hwaddr.sa_family);
                        kNetworkAdapterState adapterState = (isEnabled) ? kNETWORK_ADAPTER_STATE_ENABLED : kNETWORK_ADAPTER_STATE_DISABLED;

                        if (((adapterType & typeFilter) != 0) && (adapterState & stateFilter) != 0)
                        {
                            kTest(xkNetworkInfo_AddAdapter(info, name, id, isEnabled, isLinkRunning, adapterType, macAddress, kIP_ASSIGNMENT_UNKNOWN, &adapter));
                        }
                    }
                }
        ...
        while (!kIsNull(ifaListIt))
        {
            ...
            if ((ifaListIt->ifa_addr != kNULL) && (ifaListIt->ifa_addr->sa_family == AF_INET))
            {
                ...
                if (!kSuccess(kNetworkInfo_FindAdapterByName(info, name, &adapter)))
                {
                    ...
                    {
                        kNetworkAdapterType adapterType = kNETWORK_ADAPTER_ETHERNET;
                        kNetworkAdapterState adapterState = (isEnabled) ? kNETWORK_ADAPTER_STATE_ENABLED : kNETWORK_ADAPTER_STATE_DISABLED;
...
kFx(kStatus) xkNetwork_DefaultQueryNetInfo(kNetworkInfo netInfo, xkNetworkInfoFilter typeFilter, xkNetworkInfoFilter stateFilter)
{
