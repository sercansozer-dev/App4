...
typedef kStatus (kCall* kApiQueryNetInfoFx)(kNetworkInfo netInfo, xkNetworkInfoFilter typeFilter, xkNetworkInfoFilter stateFilter); 
...
kExtern kDx(kSize) xkApiLib_threadPoolWorkerCount;           ///< User-provided constant for thread pool workers.
...
/** 
 * Limits the number of thread pool workers in the default thread pool. 
 *
 * See: kThreadPool_Default().
 * This function is not thread-safe.
 *
 * @param   workerCount Number of workers (or 0 to use the default).
 * @return              Operation status. 
 */
kFx(kStatus) kApiLib_ConfigureThreadPool(kSize workerCount);
...
kInlineFx(kSize) kApiLib_ThreadPoolWorkerCount()                    { return xkApiLib_threadPoolWorkerCount; }               ///< Gets user-provided constant for thread pool workers.
 ...
 *
 * Use this function to receive memory leak log events from kApi.
 *
 * @public              @memberof kApiLib
