 ...
 * @public              @memberof kSerializer
 * @param   serializer  Serializer object. 
 * @return              The number of bytes left to read on the current read section.
 */
kInlineFx(k64u) kSerializer_ReadBytesLeft(kSerializer serializer)
{
    kObj(kSerializer, serializer);

    kAssert(kArrayList_Count(obj->readSections) > 0);

    return kPointer_ReadAs(kArrayList_Last(obj->readSections), k64u) - kStream_BytesRead(obj->readStream); 
}
