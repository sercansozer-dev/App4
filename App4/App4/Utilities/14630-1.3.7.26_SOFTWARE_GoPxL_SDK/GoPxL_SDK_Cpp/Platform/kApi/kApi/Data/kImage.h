 ...
 * - Advanced formats are supported; set the pixel type to kVoid and format >= kPIXEL_FORMAT_VOID_TYPE_START.
 * 
 ...
 * @return                  Operation status.
 */
kInlineFx(kStatus) kImage_ConstructEx(kImage* image, kType pixelType, kSize width, kSize height, kAlloc allocator, kAlloc valueAllocator, kMemoryAlignment valueAlignment = kALIGN_ANY)
{
    return kImage_ConstructEx(image, pixelType, kPIXEL_FORMAT_NULL, width, height, allocator, valueAllocator, valueAlignment);
}
 ...
 * @return                  Operation status.
 */
kInlineFx(kStatus) kImage_ConstructEx(kImage* image, kPixelFormat format, kSize width, kSize height, kAlloc allocator, kAlloc valueAllocator = kNULL, kMemoryAlignment valueAlignment = kALIGN_ANY)
{
    return kImage_ConstructEx(image, kTypeOf(kVoid), format, width, height, allocator, valueAllocator, valueAlignment);
}
#endif
