 ...
 *
 * @internal
 * Copyright (C) 2017-2025 by LMI Technologies Inc.
...
kFx(kThreadPool) kThreadPool_Default()
{
    ...
    kLock_Enter(sobj->lock);
    {
        ...
        {
            kSize workerCount = kApiLib_ThreadPoolWorkerCount();
            kSize threadCount = (workerCount == 0) ? kThread_ProcessorCount() : workerCount;

            if (!kSuccess(kThreadPool_Construct(&sobj->globalPool, threadCount, kNULL)))
            {
