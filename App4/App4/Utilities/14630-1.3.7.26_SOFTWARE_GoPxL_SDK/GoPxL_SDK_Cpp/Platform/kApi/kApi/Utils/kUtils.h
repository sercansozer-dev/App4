...
#if defined(K_CPP)
/**
 * Runs a process and optionally calls back with stdout/stderr output.
 *
 * @public              @memberof kFsUtils
 * @param   path        Path of the new process.
 * @param   arguments   Arguments -- kArrayList<kString>.
 * @param   outStdOut   Callback function for stdout.
 * @param   outStdErr   Callback function for stderr.
 * @param   receiver    Context pointer for callback function.
 * @param   isShell     If kTRUE, cmd.exe can be used. 
 * @param   timeout     Timeout value, in microseconds.
 * @return              Operation status.
 */
kFx(kStatus) kInvokeProcess(const kChar* path,
    kArrayList arguments=kNULL,
    kCallbackFx outStdOut=kNULL,
    kCallbackFx outStdErr=kNULL,
    kPointer receiver=kNULL,
    kBool isShell=kFALSE,
