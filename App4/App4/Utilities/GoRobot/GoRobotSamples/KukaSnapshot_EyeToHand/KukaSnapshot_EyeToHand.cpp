/**
* KukaSnapshot_EyeToHand.cpp
* An example to doing a eye-to-hand calibration on a Kuka Robot with a Fixed Mounted Gocator Snapshot Sensor.
*
* @public
...
{
    GoRobot::Matrix           initPose = GoRobot::KukaPose(230, -60, 70, 180, 30, -30).toMatrix();
    ...
    kTry
    {
        // The function EyeToHandCalibrationPoses will return a list of poses used for the calibration.
        // This is useful for taking multiple scans of an object (ball bar) mounted on the flange from different angles.
        // Note the ballbar should be mounted on the robot arm, and in the initPose should be facing the sensor.
        GoRobot::EyeToHandCalibrationPoses(rPoses.size(), initPose, rPoses.data());
        ...
        *handEye = GoRobot::CalibEyeToHandSystem(rPoses.data(), sPoses.data(), (k32s)rPoses.size());
...
* To use it, place the ballbar in the sensor's field of view, and the arm's TCP will move right above the center of one of the balls
*
* @param    system      The Gocator System object
* @param    sensor      The Gocator Sensor object
* @param    sensor      A pointer to the KukaRobotDriver object
...
kStatus MoveToBallBar(GoSystem system, GoSensor sensor, GoRobot::Driver* robotDriver,
    GoRobot::Matrix* handEye)
{
    GoRobot::Matrix   scanPose = GoRobot::KukaPose(300, 0, 300, 180, 0, 0).toMatrix();
    ...
    kTry
    {
        ...
        GoRobot::LocateEyeToHandSystem(&ballbar_in_sensor, 1, *handEye, &ballbar_in_base);
        ballbar_in_base_flipped         = ballbar_in_base           * GoRobot::KukaPose(0, 0,    0, 180, 0, 0).toMatrix();
        ballbar_in_base_flipped_above   = ballbar_in_base_flipped   * GoRobot::KukaPose(0, 0, -100,   0, 0, 0).toMatrix();
...
int main()
{
    ...
    GoRobot::Matrix   tcp = GoRobot::KukaPose(0, 100, 0 , 0,0,0).toMatrix();
    GoRobot::Matrix   handEye;
    ...
    kTry
    {
        ...
        cout << "Place the BallBar in the sensor's field of view, and press any key to continue...";
        getchar();
